...
async function uploadExcel() {
  ...
  const API_CFG_IMPORT = `http://localhost:8002/api/timetable-slots/import`;
  try {
    const res = await fetch(API_CFG_IMPORT, {
      method: "POST",
      body: formData,
    });
    ...
    if (!res.ok) {
      ...
      throw new Error(error.detail || "Lỗi khi import file cấu hình các tiết học.");
    }
    ...
    await reload(); // Load lại danh sách cấu hình và cập nhật giao diện
...
$(document).ready(function () {
  ...
  cfgSummary = $("#cfg-summary")[0];
  cfgTbody = $("#cfg-tbody")[0];
  cfgMsg = $("#cfg-msg")[0];

  selAddDay = $("#cfg-add-day")[0];
  selDelDay = $("#cfg-del-day")[0];
  selAddSession = $("#cfg-add-session")[0];
  selDelSession = $("#cfg-del-session")[0];

  inpAddSessionPeriods = $("#cfg-add-session-periods")[0];
  wrapPeriodCtrls = $("#cfg-period-controls")[0];

  selDayPerDay = $("#day-per-day")[0];
  selSessionPerDay = $("#session-per-day")[0];
  inpPeriodsPerDay = $("#periods-per-day")[0];
  btnDayMinus = $("#btn-day-minus")[0];
  btnDayPlus = $("#btn-day-plus")[0];
  btnApplyPerDay = $("#btn-apply-per-day")[0];
